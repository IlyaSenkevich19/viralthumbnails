  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          'relative flex max-h-[min(94dvh,880px)] w-full flex-col overflow-hidden border border-white/[0.09] bg-card shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_40px_120px_-56px_rgba(0,0,0,0.95)]',
          'motion-base rounded-t-[1.35rem] sm:max-w-6xl sm:rounded-[1.75rem]',
        )}
        initial={reduceMotion ? false : { opacity: 0, scale: 0.97, y: 16 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={reduceMotion ? { duration: 0 } : { ...vtSpring.enter, delay: 0.02 }}
      ...
      >
        ...
        <div className="relative min-h-[min(360px,48dvh)] flex-1 overflow-hidden">
          {refinePending ? (
            <div
              className={cn(
          ...
          <div className="flex h-full max-h-[min(calc(94dvh-10rem),78dvh)] flex-col overflow-hidden">
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-7 sm:py-7">
              <div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-[minmax(200px,280px)_minmax(0,1fr)] lg:items-start lg:gap-12 xl:grid-cols-[minmax(220px,300px)_minmax(0,1fr)]">
                <aside className="space-y-3 lg:sticky lg:top-0">
                  <div className="overflow-hidden rounded-2xl bg-muted shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_28px_80px_-48px_rgba(0,0,0,0.85)] ring-1 ring-white/[0.08]">
                    <div className="relative aspect-video w-full bg-[radial-gradient(ellipse_at_50%_0%,rgba(255,59,59,0.1)_0%,transparent_55%),rgba(0,0,0,0.45)]">
                      {previewUrl ? (
                        <div className="flex h-full w-full items-center justify-center p-2">
                          ...
                          <img
                            src={previewUrl}
                            alt={`Thumbnail being refined: ${projectTitle}`}
