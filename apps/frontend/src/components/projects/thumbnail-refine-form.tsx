...
function refineExplainerCopy(refsActiveDescription: boolean) {
  const refsDetail = refsActiveDescription
    ? 'Template / face rails on the left stream into the refinement prompt as subtle hints.'
    : 'Layer template or likeness picks before applying if you want extra layout or likeness steering.';
  return (
    <>
      <p>Drafts another thumbnail version—you can rewind through history anytime.</p>
      <p className="mt-2">{refsDetail}</p>
    </>
  );
}
      ...
      {layout === 'studio' ? (
        <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1">
          <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
            <h3 className="text-base font-semibold leading-tight tracking-tight text-foreground">What should change?</h3>
            <InfoHint
              className="shrink-0"
              buttonLabel="How thumbnail refinements work"
              helpBody={refineExplainerCopy(refsActiveDescription)}
            />
          </div>
      ...
      {layout === 'compact' ? (
        <>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1">
              ...
              <InfoHint
                className="shrink-0"
                buttonLabel="Instructions for refining this thumbnail"
                helpBody={refineExplainerCopy(refsActiveDescription)}
              />
        ...
        <div className="flex flex-wrap items-start gap-2.5">
          ...
          <div className="flex min-w-0 flex-col gap-2">
            <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
              <h3 className="text-sm font-semibold leading-tight tracking-tight text-foreground">Describe your edit</h3>
              <InfoHint
                className="shrink-0"
                buttonLabel="What this refinement affects"
                helpBody={refineExplainerCopy(refsActiveDescription)}
              />
              <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-[10px] font-medium tabular-nums text-muted-foreground ring-1 ring-white/[0.06]">
                {VARIANT_REFINE_CREDIT_COST} credit
