...
- `POST /api/thumbnails/pipeline/run-video` (multipart, async enqueue)
...
flowchart TD
  ...
  runVideoEndpoint["POST /pipeline/run-video"] --> enqueueJob
  execution --> orchestrator["ThumbnailPipelineOrchestratorService"]
  orchestrator --> persistence["PipelineProjectPersistenceService"]
```

1. `run-video` path: optional ingest (`VideoIngestionService`) + duration gate, then enqueue async job.
2. `run` path: enqueue async job and return `job_id`; runner claims and executes in background.
3. `ThumbnailPipelineExecutionService` coordinates request-shaping and final response assembly used by async jobs.
...
- `services/thumbnail-pipeline-execution.service.ts` — shared execution path used by async job execution
